...
import { useGetSingleNewsQuery, useUpdateNewsMutation} from "../../../../../redux/apps/news/api";
import useToast from "../../../../../hooks/useToast";
import { stateToHTML } from "draft-js-export-html";
import {useRouter} from "next/router";
import LoadingForButton from "../../../../../views/component/loading/LoadingForButton";

const EditNewsByAdmin = () => {
  const [imgSrc, setImgSrc] = useState(`/images/avatars/1.png`)
  const [updateNews, {isLoading, isSuccess, data, error, isError}] = useUpdateNewsMutation()
  useToast({
    isError, isSuccess, data, error, redirectUrl : '/admin/admin-config/update-news'
  })
  const router = useRouter()

  const {data: singleNews} = useGetSingleNewsQuery(router?.query?.id)
  ...
  useEffect(()=>{
    ...
    setImgSrc(singleNews?.image)

    const content = singleNews?.content
    const editorValue= convertFromHTML( content ?? '<p></p>')

    const state = ContentState.createFromBlockArray(
      editorValue.contentBlocks,
      editorValue.entityMap,
    )
  ...
  return (
    <EditorWrapper>
      <Card>
        <CardHeader title={<Typography variant={'h5'}>Edit News</Typography>} />
        <CardContent>
...
export default EditNewsByAdmin
